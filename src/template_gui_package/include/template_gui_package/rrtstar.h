...
public:
  ...
  void set_start_and_goal(Point3D _start, Point3D _goal, double time_for_all_points);
...
void RRTstar3D::set_start_and_goal(Point3D _start, Point3D _goal, double time_for_all_points)
{
  srand(time(0));
  start = _start;
  goal = _goal;
...
{
  int X = rand() % (int)(max_x - min_x +1) + (min_x);
  int Y = rand() % (int)(max_y - min_y +1) + (min_y);
  int Z = rand() % (int)(max_z - min_z +1) + (min_z);
